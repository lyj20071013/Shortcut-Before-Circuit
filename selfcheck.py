...
def check_grid_corners(n=1500, r_grid=None, d_grid=None,
                       stmts=(45, 55), spec=None):
    """相图格子的协变量。默认参数必须与 sweep.FIXED 一致，否则附录里的
    协变量表描述的不是主网格实际用的语料。"""
    r_grid = r_grid or [2, 3, 5, 8, 12, 16]
    d_grid = d_grid or DELTA_D_GRID
    spec = spec or LangSpec(n_values=512, n_entities=200)
    out = []
    for r in r_grid:
        for d in d_grid:
            dlo, dhi = dd_band(d)
            cfg = CorpusCfg(name=f"R{r}_D{d}", seed=0, p_update=0.5,
                            max_updates=1, r_old_lo=r, r_old_hi=r,
                            use_marker=False, delta_d_lo=dlo, delta_d_hi=dhi,
                            p_hist_query=0.0,
                            n_stmts_lo=stmts[0], n_stmts_hi=stmts[1])
            out.append((r, d, check(cfg, spec=spec, n=n)))
    return out

def check_stream_offsets(vocab, cfg, n=30):
    """generate_corpus 对同一 seed_offset 产出相同文档，故 Stream 每轮必须
    推进 offset。不推进则 n_workers×CHUNK 篇被反复重放，症状是训练 loss
    破解析地板而评估 loss 反向上升 —— 所有指标都在动，极难归因。"""
    def h(off):
        return {tuple(d.tokens) for d in generate_corpus(vocab, cfg, n, seed_offset=off)}
    a, b, ev = h(1000), h(1004), h(1)
    assert h(1000) == a, "同 offset 不可复现"
    assert not (a & b), f"offset 1000 与 1004 重叠 {len(a & b)} 篇"
    assert not (a & ev), f"训练流与评估集重叠 {len(a & ev)} 篇"
