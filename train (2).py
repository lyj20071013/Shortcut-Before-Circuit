...
故不存在 epoch 与过拟合。评估集与探针集固定在 seed_offset=1，与训练流
...
from probe import (EDITS, RULE_NAMES, TRUTH, attribute, causal,
                   fit_position_offset, identifiability)
...
class Stream(IterableDataset):
    """无限流。worker w 用 seed_offset = 1000 + w，与评估集(1)不相交。"""

    def __init__(self, cfg: CorpusCfg, spec: LangSpec):
    ...
    def __iter__(self):
        ...
        while True:
            for d in generate_corpus(vocab, self.cfg, 4096, seed_offset=1000 + w):
                yield d.tokens
...
def dominant_rule(probe: dict) -> str:
    """相图着色。判据是同一子集上的配对差 rate_disc - rate_truth_disc：
    模型在"规则 k 与真值分歧"的样本上更常跟 k 走，才算被 k 驱动。
    阈值 0.05 是保守占位，正文须报告着色对阈值的敏感性；identifiability
    高的规则对（frequency 在整条轴上）不能单凭这一项归因。"""
...
def train(corpus: CorpusCfg, tc: TrainCfg, mc_kw: Optional[dict] = None,
    ...
    for step in range(1, tc.total_steps + 1):
        ...
        if step in pts:
            ...
            emit(dict(kind="probe", step=step, dominant=dominant_rule(pb), **pb))
            print(f"  step {step:>6} probe dominant={dominant_rule(pb)} "
    ...
    emit(dict(kind="done", step=tc.total_steps, tokens=tok_seen,
              minutes=(time.time() - t0) / 60, **final_eval))
    logf.close()
    return final_eval
...
def main():
    ...
    tc = TrainCfg(total_steps=a.steps, batch_docs=a.batch, lr=a.lr, seed=a.seed,
                  num_workers=a.workers, out_dir=a.out, eval_docs=a.eval_docs,
                  eval_every=a.eval_every, compile=a.compile)
